...
**Objeto**: Influxos da disputa político-partidária sobre o exercício da jurisdição constitucional pelo Supremo Tribunal Federal 

**Objetivos**:

(i) Considerando o peculiar desenho institucional brasileiro, que atribui ao Presidente da República e aos partidos políticos legitimidade para a propositura de ações de controle concentrado de constitucionalidade (ADI/ADC/ADPF), questiona-se se o exercício da jurisdição constitucional pelo Supremo Tribunal Federal espelharia, mesmo que parcialmente, a disputa político-partidária ocorrida no âmbito do Congresso Nacional. 

(ii) Busca-se aferir se as ações de controle concentrado de constitucionalidade seriam instrumentos utilizados precipuamente pelos partidos políticos que compõem a Coalizão de Governo ou pelos partidos políticos da Oposição. 

(iii) Também se almeja verificar se as ações de controle concentrado de constitucionalidade ajuizadas por partidos políticos que compõem, no momento em que decisão judicial é prolatada, a Coalização de Governo possuiriam maior probabilidade de êxito em suas postulações. 

(iv) Objetiva-se, ainda, verificar se há uma diferença significtaiva entre o número de ações de controle concentrado de constitucionalidade ajuizadas por partidos políticos classificados ideologicamente como de Direita e o número de ações de controle concentrado de constitucionalidade ajuizadas pelos partidos políticos classificados ideologicamente como de Esquerda. 

(v) Por fim, busca-se aferir se Ministros do Supremo Tribunal Federal indicados por Presidentes da República classificados ideologicamente como de Esquerda ou de Direita teriam a propensão de decidir mais favoravelmente a partidos políticos que gravitam no mesmo campo ideológico. 

## Justificativa

Considerando a proeminência alcançada pelo Supremo Tribunal Federal na definição e na efetivação do desenho institucional esboçado pela Constituição de 1988, acabando por se imiscuir em searas até então compreendidas como eminentemente políticas, afigura-se relevante - e mesmo necessária - a análise de se e como o exercício da jurisdição constitucional pela nossa Corte Suprema espelha, mesmo que parcialmente, as dinâmicas inerentes às disputas político-partidárias travadas no âmbito do Congresso Nacional. 


```{r, echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics("plenario-stf.jpg")
```


## Metodolgia 

Os dados foram extraídos do [Programa Corte Aberta do Supremo Tribunal Federal](https://transparencia.stf.jus.br/extensions/corte_aberta/corte_aberta.html) e do sítio eletrônico do [Banco de Dados Legislativos do CEBRAP](https://bancodedadoslegislativos.com.br/index.php).

Os dados foram trabalhados e combinados com a finalidade de se obter a listagem de todas as ações de controle concentrado de constitucionalidade ajuizadas, desde a promulgação da Constituição de 1988, pelos Presidentes da República e pelos partidos políticos. Os partidos políticos foram classificados como pertecentes à Coalizão de Governo ou à Oposição.  

Os partidos políticos também foram classificados ideologicamente com base na classificação sugerida por Bolognesi, Ribeiro, Coldato, 2023. 

Com base nesses dados, foi sugerido um *proxy* para a mensuração do posicionamneto ideológico dos Ministros do Supremo Tribunal Federal, consistente na diferença entre o número de decisões proferidas em processos de controle concentrado de constitucionalidade (ADC/ADI/ADPF) favoráveis a partidos políticos de Esquerda e o número de decisões favoráveis a partidos de Direita. 

Tendo em vista que o número de processos ajuizados pelos partidos políticos de Direita é menor do que o número de processos ajuizados pelos partidos políticos de Esquerda (786 e 1341), foi atribuído o peso de 1,7 às decisões judiciais favorávei proferidas nos processos ajuizados pelos partidos de Direita. A fim de evitar valores negativos, a cada um dos valores foi adicionado 50 unidades. Desse modo, valores acima de 50 revelam posicionamento ideológico à Esquerda e valores abaixo de 50, posicionamento ideológico à Direita.

Desse modo, a cada um dos Ministro da Suprema Corte foi atribuída uma pontuação (número de decisões favoráveis à partidos de Esquerda - 1,7*número de decisões favoráveis a partidos de Direita)

Todos os Ministros que atuaram na Corte Suprema desde a promulgação da Constituição de 1988 foram ideologicamente classificados a partir da ideologia professada pelo Presidente da República responsável pela indicação. 

A partir dessa classificação, foram criados dois grupos: Ministros de Direita e Ministros de Esquerda.

Decidiu-se então por testar se existiria uma diferença estatisticamente significante entre as médias das pontuações atribuídas a cada um dos grupos. Ou seja, se Ministros indicados por Presidentes de Direita ou de Esquerda tenderiam a votar mais favoravlemente a partidos que estivessem no mesmo campo ideológico. 

Tendo em vista a existência de duas amostras independentes ("decisões proferidas por Ministros do STF indicados por Presidentes classificados ideologicamente como de Esquerda" e decisões proferidas por Ministros do STF indicados por Presidentes classificados ideologicamente como de Direita") e considerando que essas amostras apresentam variâncias homogêneas, optou-se pelo "Teste-t para amostras independentes com variâncias iguais".
...
## Coalizões de Governo (CEBRAP - Banco de Dados Legislativos)

```{r, echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(coalizoes)
```

## Comparativo entre as ações de controle concentrado de constitucionalidade ajuizadas por partidos políticos da Coalização de Governo (incluídas as ações ajuizadas pelos Presidentes da República) e por partidos políticos da Oposição

### Número total de ações de controle concentrado de constitucionalidade ajuizadas por partidos políticos da Coalizão de Governo e da Oposição desde a promulgação da Constituição de 1988
...
## Número de ações de controle concentrado ajuizadas por partidos políticos da Coalizão de Governo e da Oposição em cada mandato presidencial 

```{r, echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(Sumario_presidente)
```

## Distribuição das ações de controle concentrado ajuizadas por partidos políticos da Coalizão de Governo e da Oposição em cada mandato presidencial  
...
## Probabilidade de êxito das ações de controle concentrado de constitucionalidade ajuizadas por partidos políticos que compõem a Coalização de Governo no momento em que a decisão judicial é proferida
...
## **Política judicial**: uma proposta de mensuração dos posicionamentos ideológicos dos Ministros do Supremo Tribunal Federal com base nas decisões proferidas em ações de controle concentrado de constitucionalidade ajuizadas por partidos políticos e pelos Presidentes da República desde a promulgação da Constituição de 1988

Classificação ideológica dos partidos políticos brasileiros (Bolognesi, Ribeiro, Coldato, 2023)
```{r, echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics("Ideologia_partidos.png")
```

## Comparativo entre o número de ações de controle concentrado de constitucionalidade ajuizadas por partidos de Esquerda e de Direita (Classificação binária)
...
Obs: Tendo em vista a inexistência de Ministros do Supremo Tribunal Federal indicados por Presidentes classificados ideologicamente como de Centro, decidiu-se por uma divisão binária ou dicotômica (Direita-Esquerda), utilizando-se a mediana da distribuição. Desse modo, REDE, PPS e PV foram classificados como partidos de Esquerda.  


## Há diferença entre os posicionamentos ideológicos dos Ministros do Supremo Tribunal Federal indicados por Presidentes de Direita e de Esquerda? É possível aferir essa diferença utilizando como critério a diferença entre o número de decisões favoráveis a partidos de Esquerda e de Direita?
...
## Distribuição dos posicionamentos ideológicos dos Ministros do Supremo Tribunal Federal
...
## Teste de normalidade da distribuição (Teste de Shapiro-Wilk)

```{r, echo=FALSE, message=FALSE, warning=FALSE}
library(rstatix)

sumario2 %>% group_by(Presidente) %>% 
  shapiro_test(Posicionamento_ideológico)
...
É pressuposto do referido teste a distribuição normal dos dados. Em um primeiro momento, aferiu-se, por intermédio do Teste de Shapiro-Wilk, que a distribuição das observações relativas ao grupo dos Ministros indicados por Presidentes de Direita não era normal.

Após análise pormenorizada dos dados, verificou-se que a anormalidade da distribuição decorria da existência de uma observação extrema ("outlier") - Ministro Ilmar Galvão.
...
Hipótese nula (H0) não rejeitada. Inexistência, portanto, de divergência estatisticamente signficante entre os posicionamentos ideológicos adotados pelos Ministros do STF indicados por Presidentes de Direita e de Esquerda.

Anteriormente à exclusão da observação extrema ("outlier"), a divergência entre as médias dos posicionamentos ideológicos dos grupos era estatisticamente significante. Contudo,a distribuição dos dados do grupo "Direita" não era normal.

## Boxplot

```{r, echo=FALSE, warning=FALSE, message=FALSE}
boxplot <- boxplot(Posicionamento_ideológico ~ Presidente, data = sumario2, 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("stf_sede_supremo_tribunal_federal.jpg")
